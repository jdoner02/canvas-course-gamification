...
class CourseBuilder:
    ...
    @_time_operation("load_course_config")
    ...
    def _load_config_directory(self, config_path: Path) -> Dict[str, Any]:
        ...
        for filename in json_files:
            ...
            if file_path.exists():
                ...
                section_name = filename.replace(".json", "")
                config[section_name] = {}
                logger.debug(f"Configuration file not found: {filename} (using empty defaults)")
    ...
    def _apply_template_inheritance(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not template_name or template_name not in self.available_templates:
            logger.warning(f"Template '{template_name}' not found, skipping inheritance")
            return config
    ...
    def _enhance_configuration(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "gamification" in config:
            config["gamification"] = self._enhance_gamification_config(config["gamification"])
    ...
    def _enhance_modules_with_dependencies(self, modules: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for module in modules:
            ...
            for prereq_id in prerequisites:
                if prereq_id in module_map:
                    ...
                    logger.warning(f"Module {module['id']} references unknown prerequisite: {prereq_id}")
            
            module["_resolved_prerequisites"] = resolved_prereqs
            
            # Add dependency depth for sorting
            module["_dependency_depth"] = self._calculate_dependency_depth(module["id"], module_map)
    ...
    def _calculate_dependency_depth(self, module_id: str, module_map: Dict[str, Dict[str, Any]], visited: Optional[Set[str]] = None) -> int:
        ...
        if module_id in visited:
            ...
            logger.warning(f"Circular dependency detected involving module: {module_id}")
            return 0
        ...
        max_depth = 0
        for prereq_id in prerequisites:
            depth = self._calculate_dependency_depth(prereq_id, module_map, visited.copy())
            max_depth = max(max_depth, depth)
        
        return max_depth + 1

    def _enhance_gamification_config(self, gamification_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _validate_accessibility_compliance(self, config: Dict[str, Any]) -> None:
        ...
        for section in ["pages", "assignments", "modules"]:
            if section in config and config[section]:
                items = config[section].get(section, []) if isinstance(config[section], dict) else config[section]
                for item in items:
                    content_fields = ["body", "description", "content"]
                    for field in content_fields:
                        if field in item and item[field]:
                            if "<img" in item[field] and "alt=" not in item[field]:
                                issues.append(f"{section} '{item.get('name', item.get('title', 'Unknown'))}': Images missing alt text")
        ...
        for section in ["pages", "assignments"]:
            if section in config and config[section]:
                ...
                for item in items:
                    ...
                    for field in content_fields:
                        if field in item and item[field]:
                            if ("<video" in item[field] or "youtube.com" in item[field] or "vimeo.com" in item[field]):
                                # This is a simplified check - in practice, you'd want more sophisticated validation
                                if "captions" not in item[field].lower() and "cc" not in item[field].lower():
                                    issues.append(f"{section} '{item.get('name', item.get('title', 'Unknown'))}': Video content may be missing captions")
    ...
    @_time_operation("build_skill_tree")
    def build_skill_tree(self, config: Dict[str, Any]) -> SkillTree:
        ...
        for badge_data in badges_data:
            try:
                ...
                logger.warning(f"Failed to add badge {badge_data.get('id', 'Unknown')}: {e}")
        ...
        logger.info(f"Built skill tree with {len(modules)} nodes and {len(badges_data)} badges")
        return skill_tree
    ...
    def deploy_course(
        ...
        try:
            ...
            if self.deployment_strategy == DeploymentStrategy.PARALLEL:
                ...
                logger.info("Parallel deployment not yet implemented, using dependency-aware")
                deployment_results = self._deploy_dependency_aware(config, target_course_id, deployment_results)
            elif self.deployment_strategy == DeploymentStrategy.DEPENDENCY_AWARE:
                deployment_results = self._deploy_dependency_aware(config, target_course_id, deployment_results)
            else:  # SEQUENTIAL
                deployment_results = self._deploy_sequential(config, target_course_id, deployment_results)
            ...
            deployment_results["completed_at"] = self.deployment_metrics.end_time.isoformat()
            ...
            logger.info(f"Course deployment completed in {self.deployment_metrics.duration:.2f}s "
    ...
    def _validate_only_deployment(self, config: Dict[str, Any], results: Dict[str, Any]) -> Dict[str, Any]:
        """Perform validation-only deployment (no actual changes to Canvas)."""
        results["mode"] = "validation_only"
        
        # Validate each component
        for component_name in ["outcomes", "pages", "assignments", "quizzes", "modules"]:
            if component_name in config and config[component_name]:
                validation_result = self._validate_component(component_name, config[component_name])
        ...
        total_errors = sum(len(comp.get("errors", [])) for comp in results["components"].values())
        total_warnings = sum(len(comp.get("warnings", [])) for comp in results["components"].values())
        ...
        logger.info(f"Validation completed: {total_errors} errors, {total_warnings} warnings")
        return results

    def _setup_deployment_environment(self, config: Dict[str, Any], course_id: str) -> None:
    ...
    def _deploy_sequential(self, config: Dict[str, Any], course_id: str, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for component_name in deployment_order:
            if component_name in config and config[component_name]:
                ...
                try:
                    component_result = self._deploy_component(component_name, config[component_name], course_id)
                    results["components"][component_name] = {
                        "status": "completed",
                    ...
                    logger.info(f"Successfully deployed {len(component_result)} {component_name}")
                    ...
                    results["components"][component_name] = {
                        "status": "failed",
                        "items": [],
                        "errors": [{"message": str(e), "timestamp": datetime.now(timezone.utc).isoformat()}]
                    }
                    ...
                    if component_name in ["outcomes", "assignments", "pages"]:
                        raise DeploymentError(f"Critical component {component_name} failed: {e}")
            else:
                results["components"][component_name]["status"] = "skipped"
        
        return results

    def _deploy_dependency_aware(self, config: Dict[str, Any], course_id: str, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        dependencies = {
            ...
            "modules": ["outcomes", "pages", "assignments", "quizzes"]  # References everything
        }
            ...
            if component_name in config and config[component_name]:
                ...
                try:
                    ...
                    if self._is_critical_dependency(component_name, sorted_components):
                        raise DeploymentError(f"Critical dependency {component_name} failed: {e}")
            else:
                results["components"][component_name]["status"] = "skipped"
    ...
    def _is_critical_dependency(self, failed_component: str, remaining_components: List[str]) -> bool:
        ...
        remaining_set = set(remaining_components[remaining_components.index(failed_component) + 1:])
        critical_for = set(critical_dependencies.get(failed_component, []))
        
        return bool(remaining_set.intersection(critical_for))

    def _deploy_component(self, component_name: str, component_config: Dict[str, Any], course_id: str) -> List[Dict[str, Any]]:
    ...
    def _validate_component(self, component_name: str, component_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        items = component_config.get(component_name, []) if isinstance(component_config, dict) else component_config
                ...
                for item in deployed_items:
                    if "id" in item:
                        try:
                            ...
                            logger.warning(f"Could not verify {component_name} item {item.get('id', 'Unknown')}: {e}")
    ...
    def _rollback_deployment(self, results: Dict[str, Any], course_id: str) -> None:
        ...
        for component_name, component_result in reversed(list(results["components"].items())):
            if component_result["status"] == "completed":
                try:
                    deployed_items = component_result["items"]
                    for item in deployed_items:
                        if "id" in item:
                            ...
                            logger.debug(f"Rolling back {component_name} item {item['id']}")
        ...
        for module_config in modules:
            try:
                ...
                if self.deployment_mode == DeploymentMode.DRY_RUN:
                    ...
                    logger.info(f"[DRY RUN] Would create module: {module_config['name']}")
                else:
    ...
    def _validate_module_accessibility(self, module_config: Dict[str, Any]) -> None:
        ...
        if description and not any(tag in description.lower() for tag in ["<h1", "<h2", "<h3"]):
            logger.warning(f"Module '{module_config['name']}' description may lack proper heading structure")
    ...
    def _add_module_item(
        ...
        if self.deployment_mode == DeploymentMode.DRY_RUN:
            logger.info(f"[DRY RUN] Would add item '{item_config.get('title', 'Untitled')}' to module {module_id}")
            return {"id": f"mock_item_{module_id}", "title": item_config.get("title", "Untitled Item")}
        ...
        for assignment_config in assignments:
            try:
                ...
                if self.deployment_mode == DeploymentMode.DRY_RUN:
                    ...
                    logger.info(f"[DRY RUN] Would create assignment: {assignment_config['name']}")
                else:
    ...
    def _validate_assignment_accessibility(self, assignment_config: Dict[str, Any]) -> None:
        """Validate assignment for accessibility compliance."""
        description = assignment_config.get("description", "")
        
        # Check for images without alt text
        if "<img" in description and "alt=" not in description:
            logger.warning(f"Assignment '{assignment_config['name']}' may have images without alt text")
        
        # Check for links without descriptive text
        if description.count("<a") > description.count("</a>"):
            logger.warning(f"Assignment '{assignment_config['name']}' may have malformed links")
    ...
    def _deploy_pages(
        ...
        for page_config in pages:
            try:
                ...
                error_msg = f"Failed to deploy page {page_config.get('title', 'Unknown')}: {e}"
                logger.error(error_msg)
                self.deployment_metrics.errors.append(error_msg)
                raise
    ...
    def _validate_page_accessibility(self, page_config: Dict[str, Any]) -> None:
        ...
        if body and not any(tag in body.lower() for tag in ["<h1", "<h2", "<h3"]):
            logger.warning(f"Page '{page_config['title']}' may lack proper heading structure")
        
        # Check for tables without headers
        if "<table" in body and "<th" not in body:
            logger.warning(f"Page '{page_config['title']}' contains tables without headers")
    ...
    def _deploy_quizzes(
        ...
        for quiz_config in quizzes:
            try:
                ...
                error_msg = f"Failed to deploy quiz {quiz_config.get('title', 'Unknown')}: {e}"
                logger.error(error_msg)
                self.deployment_metrics.errors.append(error_msg)
                raise
    ...
    def _deploy_outcomes(
        ...
        for outcome_config in outcomes:
            try:
                if self.deployment_mode == DeploymentMode.DRY_RUN:
                    logger.info(f"[DRY RUN] Would process outcome: {outcome_config.get('title', 'Unknown')}")
                    results.append({
                    ...
                    logger.info(f"Processing outcome: {outcome_config.get('title', 'Unknown')}")
        ...
        if "modules" in config and config["modules"]:
            ...
            for module in modules:
                ...
                if not module.get("name"):
                    errors.append(f"Module {module.get('id', 'Unknown')} missing required 'name' field")
                ...
                if isinstance(prerequisites, list):
                    for prereq in prerequisites:
                        if prereq not in module_ids:
                            warnings.append(f"Module {module['id']} references unknown prerequisite: {prereq}")

                # UDL validation
                if not module.get("description"):
                    warnings.append(f"Module {module['id']} missing description (UDL: multiple means of representation)")
        ...
        if "assignments" in config and config["assignments"]:
            ...
            for assignment in assignments:
                ...
                if not assignment.get("name"):
                    errors.append(f"Assignment {assignment.get('id', 'Unknown')} missing required 'name' field")
                
                # Accessibility checks
                if assignment.get("description"):
                    desc = assignment["description"]
                    if "<img" in desc and "alt=" not in desc:
                        warnings.append(f"Assignment {assignment['id']} may have images without alt text")

            # Check if module items reference valid assignments
            if "modules" in config and config["modules"]:
                for module in config["modules"].get("modules", []):
                    for item in module.get("items", []):
                        if (item.get("type") == "Assignment" and 
                            item.get("id") not in assignment_ids):
                            warnings.append(f"Module {module['id']} references unknown assignment: {item.get('id')}")
        ...
        if "gamification" in config:
            ...
            if gamification.get("enable_skill_tree", False):
                if not config.get("skill_tree"):
                    warnings.append("Skill tree enabled but no skill tree configuration provided")

        # Performance considerations
        total_items = sum(len(config.get(section, {}).get(section, [])) 
                         for section in ["modules", "assignments", "pages", "quizzes"] 
                         if section in config)
        if total_items > 100:
            warnings.append(f"Large course detected ({total_items} items). Consider breaking into smaller courses for better performance.")
