...
class GamificationEngine:
    ...
    def onboard_student(
        ...
        player_class = (
            preferences.get("player_class", PlayerClass.ENGINEER)
            if preferences
            else PlayerClass.ENGINEER
        )
    ...
    def award_xp(
        ...
        if profile.rpg_active:
            ...
            if rpg_player:
                if (
                    ...
                    and rpg_player["class"] == PlayerClass.ENGINEER
                ):
                    total_multiplier += 0.2  # Engineers get bonus for problem solving
                elif (
                    source == "peer_teaching"
                    and rpg_player["class"] == PlayerClass.PURE_MATHEMATICIAN
                ):
        ...
        if profile.mentor_status:
            ...
            if (
                mentor_stats
                and mentor_stats["profile"]["average_satisfaction_rating"] > 4.0
            ):
        ...
        if profile.rpg_active:
            ...
            if activity_type in ["matrix_operations", "determinant_calculation"]:
                self.rpg_system.award_skill_experience(
                    student_id, SkillTree.MATRIX_MASTERY, skill_xp
                )
            elif activity_type in ["vector_operations", "dot_product"]:
                self.rpg_system.award_skill_experience(
                    student_id, SkillTree.VECTOR_OPERATIONS, skill_xp
                )
