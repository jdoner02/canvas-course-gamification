...
class WorldBuilder:
    ...
    def __init__(self):
        ...
        self.global_discoveries: Dict[str, Dict] = (
            {}
        )  # Notable discoveries by all students
        ...
        self.building_costs = {
            BuildingType.STUDY_DESK: {
                ResourceType.VECTOR_CRYSTALS: 10,
                ResourceType.MATRIX_STONES: 5,
            },
            BuildingType.CALCULATION_LAB: {
                ResourceType.MATRIX_STONES: 20,
                ResourceType.DETERMINANT_GEMS: 10,
            },
            BuildingType.MATRIX_FORGE: {
                ResourceType.DETERMINANT_GEMS: 15,
                ResourceType.LINEAR_ESSENCE: 5,
            },
            BuildingType.VECTOR_GARDEN: {
                ResourceType.VECTOR_CRYSTALS: 25,
                ResourceType.TRANSFORMATION_ENERGY: 3,
            },
            BuildingType.EIGEN_OBSERVATORY: {
                ResourceType.EIGENVALUE_SHARDS: 20,
                ResourceType.KNOWLEDGE_FRAGMENTS: 5,
            },
            BuildingType.KNOWLEDGE_LIBRARY: {
                ResourceType.KNOWLEDGE_FRAGMENTS: 15,
                ResourceType.LINEAR_ESSENCE: 10,
            },
            BuildingType.COLLABORATION_HALL: {
                ResourceType.MATRIX_STONES: 30,
                ResourceType.VECTOR_CRYSTALS: 30,
            },
            BuildingType.PRACTICE_ARENA: {
                ResourceType.DETERMINANT_GEMS: 25,
                ResourceType.EIGENVALUE_SHARDS: 15,
            },
        }
    ...
    def create_study_base(
        self,
        student_id: str,
        base_name: str,
        dimension: WorldDimension = WorldDimension.VECTOR_SPACE,
    ) -> str:
        ...
        logger.info(
            f"Created study base '{base_name}' for student {student_id} in {dimension.value}"
        )
        self._save_data()
        return base_id
    ...
    def construct_building(
        ...
        for building in base.buildings.values():
            if building.position == position:
                logger.warning(
                    f"Position {position} already occupied in base {base_id}"
                )
                return None
        ...
        for resource_type, cost in required_resources.items():
            if base.resources.get(resource_type, 0) < cost:
                logger.warning(
                    f"Insufficient {resource_type.value} to build {building_type.value}"
                )
                return None
        ...
        logger.info(
            f"Constructed {building_type.value} at position {position} in base {base_id}"
        )
        self._save_data()
        return building_id
    ...
    def upgrade_building(self, base_id: str, building_id: str) -> bool:
        ...
        for resource_type, cost in upgrade_cost.items():
            if base.resources.get(resource_type, 0) < cost:
                logger.warning(
                    f"Insufficient resources to upgrade building {building_id}"
                )
                return False
        ...
        logger.info(
            f"Upgraded {building.building_type.value} to level {building.level}"
        )
        self._save_data()
        return True

    def gather_resources(
        self, student_id: str, dimension: WorldDimension, duration_minutes: int = 30
        ...
        for resource_type in ResourceType:
            if self._can_find_in_dimension(resource_type, dimension):
                spawn_rate = (
                    self.resource_spawn_rates[resource_type] * efficiency_multiplier
                )
                ...
                for _ in range(base_attempts):
                    if random.random() < spawn_rate:
                        ...
                        gathered[resource_type] = (
                            gathered.get(resource_type, 0) + quantity
                        )
    ...
    def _can_find_in_dimension(
        self, resource: ResourceType, dimension: WorldDimension
    ) -> bool:
        """Check if a resource can be found in a specific dimension"""
        dimension_resources = {
            WorldDimension.VECTOR_SPACE: [
                ResourceType.VECTOR_CRYSTALS,
                ResourceType.LINEAR_ESSENCE,
            ],
            WorldDimension.MATRIX_REALM: [
                ResourceType.MATRIX_STONES,
                ResourceType.DETERMINANT_GEMS,
            ],
            WorldDimension.TRANSFORMATION_GALAXY: [
                ResourceType.TRANSFORMATION_ENERGY,
                ResourceType.LINEAR_ESSENCE,
            ],
            WorldDimension.EIGENVALUE_UNIVERSE: [
                ResourceType.EIGENVALUE_SHARDS,
                ResourceType.KNOWLEDGE_FRAGMENTS,
            ],
            WorldDimension.ABSTRACT_ALGEBRA_VOID: [
                ResourceType.KNOWLEDGE_FRAGMENTS
            ],  # Rare but all types possible
        }

        return (
            resource in dimension_resources.get(dimension, [])
            or dimension == WorldDimension.ABSTRACT_ALGEBRA_VOID
        )

    def create_exploration_quest(
        self,
        title: str,
        description: str,
        quest_type: QuestType,
        ...
        objectives = self._generate_quest_objectives(
            quest_type, target_concept, difficulty
        )
    ...
    def _generate_quest_objectives(
        self, quest_type: QuestType, concept: str, difficulty: int
    ) -> List[str]:
    ...
    def update_quest_progress(
        self, student_id: str, quest_id: str, objective_index: int, progress: float
    ) -> bool:
        ...
        if all(
            quest.progress.get(f"obj_{i}", 0) >= 1.0
            for i in range(len(quest.objectives))
        ):
            self._complete_quest(student_id, quest_id)

        logger.info(
            f"Updated quest {quest_id} objective {objective_index} to {progress * 100}%"
        )
        self._save_data()
        return True
    ...
    def _generate_starting_quests(self):
        ...
        for title, desc, quest_type, concept, dimension, difficulty in starter_quests:
            self.create_exploration_quest(
                title, desc, quest_type, concept, dimension, difficulty
            )
    ...
    def get_student_world_info(self, student_id: str) -> Optional[Dict]:
        ...
        return {
            ...
            "available_quests": len(
                [q for q in self.exploration_quests.values() if not q.completion_time]
            ),
        }
...
if __name__ == "__main__":
    ...
    base_id = world.create_study_base(
        "student_alice", "Alice's Math Fortress", WorldDimension.VECTOR_SPACE
    )

    # Construct some buildings
    lab_id = world.construct_building(
        base_id, BuildingType.CALCULATION_LAB, (5, 5), "Alice's Research Lab"
    )
    ...
    if available_quests:
        ...
        world.update_quest_progress(
            "student_alice", quest_id, 0, 1.0
        )  # Complete first objective
        world.update_quest_progress(
            "student_alice", quest_id, 1, 0.5
        )  # Half complete second
