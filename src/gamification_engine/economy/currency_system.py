...
class CurrencyManager:
        ...
        for currency_type, base_amount in self.earning_rates[activity].items():
            ...
            if daily_earned.get(currency_type, 0) + amount <= daily_limit:
                ...
                transaction = Transaction(
                    ...
                    description=f"Earned from {activity}"
    ...
    def transfer_currency(
        ...
        """
        if self.spend_currency(
            from_student, currency_type, amount, f"Transfer to {to_student}: {reason}"
        ):
    ...
    def _get_daily_earnings(self, student_id: str) -> Dict[CurrencyType, int]:
        ...
        for transaction in self.transactions:
            if (
                ...
                daily_earnings[currency] = (
                    daily_earnings.get(currency, 0) + transaction.amount
                )

        return daily_earnings

    def get_leaderboard(
        self, currency_type: CurrencyType, limit: int = 10
    ) -> List[Dict]:
        """
    ...
    def apply_economic_event(
        self, event_type: str, multiplier: float, duration_hours: int
    ):
        """
        ...
        print(
            f"🎉 Economic Event: {event_type} - {multiplier}x earnings for {duration_hours} hours!"
        )
    ...
    def export_data(self) -> Dict:
        ...
        return {
            ...
            "earning_rates": {
                k: {ct.value: amt for ct, amt in v.items()}
                for k, v in self.earning_rates.items()
            },
            "daily_limits": {
                ct.value: limit for ct, limit in self.daily_limits.items()
            },
        }
...
if __name__ == "__main__":
    ...
    earned = currency_manager.award_currency(
        "alice_123", "problem_solved_hard", multiplier=1.5, bonus_reason="Weekend Bonus"
    )
    ...
    print(
        f"Alice's balance: Coins={balance.coins}, Gems={balance.gems}, Elements={balance.elements}"
    )

    # Test spending
    success = currency_manager.spend_currency(
        "alice_123", CurrencyType.COINS, 30, "Bought vector visualization tool"
    )
    print(f"Purchase successful: {success}")

    # Test transfer
    currency_manager.award_currency("bob_456", "problem_solved_easy")
    transfer_success = currency_manager.transfer_currency(
        "alice_123", "bob_456", CurrencyType.COINS, 10, "Helping with homework"
    )
