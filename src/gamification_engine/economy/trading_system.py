...
class MarketplaceItem:
    ...
    def get_market_value(self) -> Dict[CurrencyType, int]:
        ...
        return {
            currency: int(value * multiplier)
            for currency, value in self.base_value.items()
        }
...
class Marketplace:
    ...
    def _create_item_copy(
        self, template_item_id: str, new_owner: str
    ) -> MarketplaceItem:
        ...
        self.market_history.append(
            {
                ...
                "currency_traded": {
                    **offer.offered_currency,
                    **offer.requested_currency,
                },
...
class AuctionHouse:
    ...
    def place_bid(
        self, listing_id: str, bidder_id: str, bid_amount: Dict[CurrencyType, int]
    ) -> bool:
        ...
        bidder_balance = self.marketplace.currency_manager.get_student_balance(
            bidder_id
        )
        for currency_type, amount in bid_amount.items():
        ...
        if auction.current_bidder:
            for currency_type, amount in auction.current_bid.items():
                prev_balance = self.marketplace.currency_manager.get_student_balance(
        ...
        auction.bid_history.append(
            {"bidder": bidder_id, "amount": bid_amount, "timestamp": datetime.now()}
        )
    ...
    def _complete_auction(
        self, listing_id: str, winner_id: str, final_price: Dict[CurrencyType, int]
    ):
        ...
        for currency_type, amount in final_price.items():
            ...
            # Pay the seller
            seller_balance = self.marketplace.currency_manager.get_student_balance(
                auction.seller_id
            )
            seller_balance.add_currency(currency_type, amount)

        # Transfer item to winner
        winner_inventory = self.marketplace.student_inventories.setdefault(
            winner_id, []
        )
    ...
    def check_expired_auctions(self):
        ...
        for listing_id in expired_auctions:
            ...
            if auction.current_bidder:
                ...
                self._complete_auction(
                    listing_id, auction.current_bidder, auction.current_bid
                )
            else:
                # No bids, return item to seller
                seller_inventory = self.marketplace.student_inventories.setdefault(
                    auction.seller_id, []
                )
