...
class PerformanceMetrics:
    ...
    streak_best: int = 0
    difficulty_progression: List[Tuple[datetime, DifficultyLevel]] = field(default_factory=list)
...
class AdaptiveLearningEngine:
    ...
    def __init__(self):
        ...
        self.performance_metrics: Dict[str, Dict[ConceptCategory, PerformanceMetrics]] = {}
    ...
    def get_performance_metrics(self, student_id: str, concept: ConceptCategory) -> PerformanceMetrics:
    ...
    def record_activity(self, student_id: str, activity_type: str, 
                       concept: ConceptCategory, difficulty: DifficultyLevel,
                       success: bool, time_spent: float, mistakes: List[str] = None):
        ...
        if mistakes:
            for mistake in mistakes:
                metrics.mistake_patterns[mistake] = metrics.mistake_patterns.get(mistake, 0) + 1
    ...
    def _update_learning_profile(self, profile: LearningProfile, concept: ConceptCategory,
                               success: bool, time_spent: float, difficulty: DifficultyLevel):
        ...
        profile.confidence_level = max(0.0, min(1.0, profile.confidence_level + confidence_change))
        
        # Update motivation (decreases with failures, increases with successes)
        motivation_change = 0.02 if success else -0.03
        profile.motivation_level = max(0.0, min(1.0, profile.motivation_level + motivation_change))
    ...
    def detect_learning_style(self, student_id: str) -> Dict[LearningStyle, float]:
        ...
        student_activities = [a for a in self.recent_activities if a["student_id"] == student_id]
        ...
        visual_success = sum(1 for a in student_activities 
        ...
        logical_success = sum(1 for a in student_activities 
        ...
        normalized_scores = {style: score/total for style, score in style_scores.items()}
    ...
    def get_adaptive_recommendation(self, student_id: str) -> AdaptiveRecommendation:
        ...
        # Generate reasoning
        reasoning = self._generate_reasoning(student_id, concept, difficulty, learning_style)
    ...
    def _calculate_optimal_difficulty(self, student_id: str, concept: ConceptCategory) -> DifficultyLevel:
    ...
    def _calculate_recommendation_confidence(self, student_id: str, concept: ConceptCategory) -> float:
        ...
        overall_confidence = (data_confidence + consistency_confidence + recency_confidence) / 3
        return round(overall_confidence, 2)
    
    def _calculate_performance_variance(self, student_id: str, concept: ConceptCategory) -> float:
        """Calculate variance in recent performance"""
        # Simplified variance calculation
        student_activities = [a for a in self.recent_activities 
    ...
    def _generate_reasoning(self, student_id: str, concept: ConceptCategory,
                          difficulty: DifficultyLevel, learning_style: LearningStyle) -> str:
        ...
        skill_level = profile.skill_levels.get(concept, 0.5)
        if skill_level < 0.4:
            reasoning_parts.append(f"Focusing on {concept.value.replace('_', ' ')} to build foundational skills")
        elif skill_level > 0.8:
            reasoning_parts.append(f"Advancing in {concept.value.replace('_', ' ')} to maintain momentum")
        else:
            reasoning_parts.append(f"Practicing {concept.value.replace('_', ' ')} for skill development")
        ...
        if difficulty == DifficultyLevel.TUTORIAL:
            ...
            reasoning_parts.append(f"Optimal difficulty level for your current progress")
        
        # Learning style reasoning
        reasoning_parts.append(f"Using {learning_style.value} approach based on your preferences")
        
        # Performance-based reasoning
        if metrics.success_rate > 0.8:
            reasoning_parts.append("Your strong performance suggests you're ready for this challenge")
        elif metrics.success_rate < 0.5:
            reasoning_parts.append("This will help reinforce concepts you're still mastering")
...
if __name__ == "__main__":
    ...
    for concept, difficulty, success, time_spent, mistakes in activities:
        engine.record_activity(student_id, "problem_solving", concept, difficulty, success, time_spent, mistakes)
    
    # Get learning profile
    profile = engine.get_learning_profile(student_id)
    print(f"Student profile - Confidence: {profile.confidence_level:.2f}, Motivation: {profile.motivation_level:.2f}")
