...
class Skill:
    ...
    specialization_bonus: Dict[MathematicalSpecialization, float] = field(default_factory=dict)
    ...
    def add_experience(self, xp_gained: int, specialization: MathematicalSpecialization) -> Dict[str, Any]:
        ...
        # Check for level ups
        while self.current_xp >= self.xp_to_next_level and self.current_level < self.max_level:
            ...
            if self.current_level < self.max_level:
                self.xp_to_next_level = self.calculate_xp_requirement(self.current_level + 1)
        ...
        return {
            ...
            "overflow_xp": self.current_xp if self.current_level >= self.max_level else 0
        }
    ...
    def prestige_reset(self) -> Dict[str, Any]:
        ...
        prestige_abilities = [f"{self.skill_id}_prestige_{len([a for a in self.unlocked_abilities if 'prestige' in a]) + 1}"]
        ...
        return {
            ...
            "xp_bonus_multiplier": 1.1 ** len(prestige_abilities),  # 10% bonus per prestige
            "status": "success"
        }
    ...
    def get_specialization_bonuses(self) -> Dict[str, float]:
        ...
        bonuses = {
            ...
            MathematicalSpecialization.INTERDISCIPLINARY: {
                ...
                category: 1.1 for category in SkillCategory
            }
        }
        return bonuses.get(self.specialization, {})

    def award_experience(self, skill_id: str, xp_amount: int, source: str = "unknown") -> Dict[str, Any]:
    ...
    def get_next_ability_unlock(self) -> Optional[Dict[str, Any]]:
        ...
        for skill in self.skills.values():
            ...
            for level in unlock_levels:
                ...
                if skill.current_level < level and ability_id not in skill.unlocked_abilities:
...
class PlayerProfileManager:
    ...
    def _create_skill_templates(self) -> Dict[str, Skill]:
        ...
        skills = {
            # Vector Operations
            "vector_basics": Skill(
                "vector_basics", "Vector Fundamentals", 
            ...
            # Matrix Mastery
            "matrix_basics": Skill(
                "matrix_basics", "Matrix Fundamentals",
            ...
            "determinants": Skill(
                "determinants", "Determinant Computation",
            ...
            "eigenvectors": Skill(
                "eigenvectors", "Eigenvector Analysis",
        ...
        for skill in skills.values():
            if skill.category == SkillCategory.APPLICATIONS:
                ...
                skill.specialization_bonus[MathematicalSpecialization.APPLIED_MATHEMATICIAN] = 1.2
            elif skill.category == SkillCategory.PROOF_TECHNIQUES:
                skill.specialization_bonus[MathematicalSpecialization.PURE_MATHEMATICIAN] = 1.3
            elif skill.category == SkillCategory.COMPUTATIONAL_SKILLS:
                skill.specialization_bonus[MathematicalSpecialization.DATA_SCIENTIST] = 1.25
    ...
    def _create_ability_templates(self) -> Dict[str, Ability]:
        ...
        return {
            "quick_learner": Ability(
                ...
                AbilityType.PASSIVE, "", 10, 0.1
            ),
            "problem_solver": Ability(
                "problem_solver", "Problem Solver",
                "Reduce hint penalties by 25%",
                AbilityType.PASSIVE, "", 25, 0.25
            ),
            "mentor": Ability(
                "mentor", "Mentor",
                "Gain XP when helping other students",
                AbilityType.SOCIAL, "", 50, 0.5
            ),
            "focused_mind": Ability(
                "focused_mind", "Focused Mind",
                "Double XP for next 30 minutes (once per day)",
                AbilityType.ACTIVE, "", 75, 2.0, 86400  # 24 hour cooldown
            )
        }

    def create_player(self, student_id: str, display_name: str, 
                     specialization: MathematicalSpecialization) -> PlayerProfile:
    ...
    def award_xp(self, student_id: str, skill_id: str, xp_amount: int, 
                 source: str = "problem_solving") -> Dict[str, Any]:
    ...
    def get_leaderboard(self, metric: str = "total_xp", limit: int = 10) -> List[Dict[str, Any]]:
        """Get leaderboard for various metrics"""
        if metric == "total_xp":
            sorted_players = sorted(self.profiles.values(), 
                                  key=lambda p: p.total_xp, reverse=True)
        elif metric == "total_levels":
            sorted_players = sorted(self.profiles.values(),
                                  key=lambda p: p.total_levels, reverse=True)
        elif metric == "streak_days":
            sorted_players = sorted(self.profiles.values(),
                                  key=lambda p: p.streak_days, reverse=True)
        else:
...
if __name__ == "__main__":
    ...
    # Create test players
    alice = manager.create_player("alice_123", "Alice Engineer", MathematicalSpecialization.ENGINEER)
    bob = manager.create_player("bob_456", "Bob DataSci", MathematicalSpecialization.DATA_SCIENTIST)
    ...
    print(f"Alice gained {result['xp_gained']} XP in Vector Basics (bonus: {result['bonus_multiplier']:.1f}x)")
    ...
    print(f"Bob gained {result['xp_gained']} XP in Matrix Basics (bonus: {result['bonus_multiplier']:.1f}x)")
    
    # Show next ability unlock
    next_unlock = alice.get_next_ability_unlock()
    if next_unlock:
        print(f"\n🔮 Alice's next unlock: {next_unlock['ability_preview']}")
        print(f"   Needs {next_unlock['levels_needed']} more levels in {next_unlock['skill_name']}")
