...
class PetStats:
    ...
    def calculate_battle_power(self) -> int:
        ...
        return (
            self.health + self.attack + self.defense + self.speed + self.intelligence
        ) // 5
...
class MathematicalPet:
    ...
    def update_care_stats(self):
        ...
        # Apply decay
        self.stats.hunger = max(
            0, self.stats.hunger - int(hours_since_fed * hunger_decay)
        )
        self.stats.energy = max(
            0, self.stats.energy - int(hours_since_played * energy_decay)
        )
        self.stats.cleanliness = max(
            0, self.stats.cleanliness - int(hours_since_cleaned * cleanliness_decay)
        )

        # Happiness affected by all care stats
        care_average = (
            self.stats.hunger + self.stats.energy + self.stats.cleanliness
        ) / 3
        if care_average < 50:
    ...
    def feed(self, food_type: str = "basic") -> Dict[str, Any]:
        ...
        if "intelligence" in effects:
            self.stats.intelligence = min(
                100, self.stats.intelligence + effects["intelligence"]
            )
    ...
    def check_evolution(self) -> Optional[Dict[str, Any]]:
        ...
        can_evolve = (
            self.days_since_hatch >= current_requirements["days"]
            and care_average >= current_requirements["care_average"]
            and self.learning_consistency
            >= current_requirements.get("learning_consistency", 0)
        )
    ...
    def _learn_evolution_ability(self) -> Optional[str]:
        ...
        if self.stats.intelligence >= max(
            self.stats.attack, self.stats.defense, self.stats.speed
        ):
    ...
    def _generate_feeding_message(self) -> str:
        ...
        if mood in [PetMood.ECSTATIC, PetMood.HAPPY]:
            ...
            return (
                f"{self.name} enjoys the meal while pondering linear transformations."
            )
        else:
    ...
    def _generate_battle_preview(
        self, opponent: "MathematicalPet", win_prob: float
    ) -> str:
...
class PetCompanionSystem:
            ...
            self.adoption_center.append(
                {
                    ...
                    "personality": random.choice(
                        ["friendly", "curious", "energetic", "calm", "playful"]
                    ),
    ...
    def adopt_pet(
        self, student_id: str, adoption_id: str, pet_name: str
    ) -> Dict[str, Any]:
