...
class PvPBattleEngine:
    ...
    def queue_for_battle(
        self, student_id: str, battle_type: BattleType, preferred_rank_range: int = 200
    ) -> bool:
        ...
        for i, player1 in enumerate(self.battle_queue):
            for j, player2 in enumerate(self.battle_queue[i + 1 :], i + 1):
                if player1["battle_type"] == player2["battle_type"]:
                    ...
                    max_diff = max(
                        player1["preferred_rank_range"], player2["preferred_rank_range"]
                    )
    ...
    def _create_battle(self, participants: List[str], battle_type: BattleType) -> str:
        ...
        logger.info(
            f"Created {battle_type.value} battle {battle_id} with {len(participants)} participants"
        )
        return battle_id
    ...
    def complete_battle(
        ...
        for participant in battle.participants:
            ...
            if participant == winner_id:
                ...
                profile.longest_win_streak = max(
                    profile.longest_win_streak, profile.current_win_streak
                )
            else:
                profile.losses += 1
            ...
            total_score = profile.average_score * (
                profile.total_battles - 1
            ) + participant_scores.get(participant, 0)
    ...
    def get_competitor_stats(self, student_id: str) -> Optional[Dict]:
        ...
        recent_battles = [
            battle
            for battle in self.battle_results.values()
        ...
        return {
            ...
            "is_in_queue": any(
                entry["student_id"] == student_id for entry in self.battle_queue
            ),
...
class TournamentManager:
    ...
    def generate_bracket(self, tournament_id: str) -> bool:
        ...
        logger.info(
            f"Generated {tournament.format.value} bracket for tournament {tournament.name}"
        )
        return True
    ...
    def _generate_round_robin_bracket(self, participants: List[str]) -> Dict:
        ...
        bracket = {
            ...
            "current_round": 0,
            "standings": {
                p: {"wins": 0, "losses": 0, "points": 0} for p in participants
            },
        }
...
if __name__ == "__main__":
    ...
    print(
        f"✅ Alice's Competition Stats: Rank = {alice_stats['profile']['current_rank']}, Points = {alice_stats['profile']['rank_points']}"
    )

    # Get leaderboard
    leaderboard = engine.get_leaderboard()
    print(
        f"🏆 Competition Leaderboard: {[(id, profile.rank_points) for id, profile in leaderboard]}"
    )
