...
class PeerTeachingEngine:
    ...
    def __init__(self):
        ...
        self.mentor_availability: Dict[str, List[str]] = (
            {}
        )  # mentor_id -> available times
    ...
    def request_help(
        ...
        student_id: str,
        topic: str,
        preferred_method: TeachingMethod,
        urgency: str = "normal",
    ) -> str:
        ...
        if matched_mentor:
            self._create_teaching_session(
                matched_mentor, [student_id], topic, preferred_method
            )
            ...
            if topic in profile.strong_topics:
                ...
                if method in profile.teaching_methods:
                    ...
                    if mentor_id not in self.active_sessions:
                        score = profile.average_satisfaction_rating + (
                            profile.mentorship_points / 1000
                        )
    ...
    def _create_teaching_session(
        self, mentor_id: str, mentee_ids: List[str], topic: str, method: TeachingMethod
    ) -> str:
    ...
    def end_teaching_session(
        ...
        avg_satisfaction = (
            sum(mentee_ratings) / len(mentee_ratings) if mentee_ratings else 0
        )
        ...
        total_ratings = mentor_profile.average_satisfaction_rating * (
            mentor_profile.total_sessions - 1
        )
        total_ratings += avg_satisfaction
        mentor_profile.average_satisfaction_rating = (
            total_ratings / mentor_profile.total_sessions
        )
        ...
        logger.info(
            f"Ended teaching session {session_id}. Mentor earned {session.mentor_xp_earned} XP"
        )
        self._save_data()
        return True
    ...
    def _check_mentorship_level_up(self, mentor_id: str):
        ...
        # Level up criteria
        if (
            profile.mentorship_points >= 10000
            and profile.average_satisfaction_rating >= 4.5
            and profile.total_sessions >= 50
        ):
            profile.mentorship_level = MentorshipLevel.MASTER_TEACHER
        elif (
            profile.mentorship_points >= 5000
            and profile.average_satisfaction_rating >= 4.0
            and profile.total_sessions >= 25
        ):
            profile.mentorship_level = MentorshipLevel.EXPERT_MENTOR
        elif (
            profile.mentorship_points >= 2000
            and profile.average_satisfaction_rating >= 3.5
            and profile.total_sessions >= 10
        ):
    ...
    def create_mentorship_chain(self, chain_name: str, participants: List[str]) -> str:
        ...
        sorted_participants = sorted(
            participants,
            key=lambda x: self.mentor_profiles.get(
                x, MentorProfile(x)
            ).mentorship_points,
            reverse=True,
        )
        ...
        chain = MentorshipChain(
            chain_id=chain_id, chain_name=chain_name, participants=sorted_participants
        )

        self.mentorship_chains[chain_id] = chain

        logger.info(
            f"Created mentorship chain: {chain_name} with {len(participants)} participants"
        )
        self._save_data()
        return chain_id

    def get_mentor_leaderboard(
        self, limit: int = 10
    ) -> List[Tuple[str, MentorProfile]]:
    ...
    def get_mentor_stats(self, mentor_id: str) -> Optional[Dict]:
        ...
        recent_sessions = [
            session
            for session in self.teaching_sessions.values()
...
if __name__ == "__main__":
    ...
    request_id = engine.request_help(
        "charlie_beginner", "Matrix Multiplication", TeachingMethod.ONE_ON_ONE, "high"
    )
    ...
    print(
        f"🏆 Mentor Leaderboard: {[(id, profile.mentorship_points) for id, profile in leaderboard]}"
    )
