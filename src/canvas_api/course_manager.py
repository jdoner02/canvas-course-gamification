...
class CourseManager:
    ...
    def deploy_course_structure(
        ...
        try:
            logger.info(
                f"Starting course deployment with validation level: {validation_level.value}"
            )
            result.status = DeploymentStatus.IN_PROGRESS

            # Phase 1: Validation
            if validation_level != ValidationLevel.BASIC:
                validation_results = self._validate_course_data(
                    course_data, validation_level
                )
                ...
                critical_failures = [
                    ...
                    if not vr.is_valid
                    and any(issue.severity == "critical" for issue in vr.issues)
                ]

                if critical_failures:
                    result.status = DeploymentStatus.FAILED
                    logger.error(
                        f"Critical validation failures found: {len(critical_failures)}"
                    )
                    return result
            ...
            result.performance_metrics = {
                ...
                "success_rate": (
                    ...
                    / (len(result.created_items) + len(result.failed_items))
                    if (result.created_items or result.failed_items)
                    else 0
                ),
            }
        ...
        for i in range(0, len(modules_data), batch_size):
            ...
            for module_config in batch:
                try:
                    ...
                    # Create module
                    created_module = self.client.create_module(
                        course_id, **module_config
                    )
                    ...
                    logger.info(
                        f"Created module: {module_config.get('name', 'Unnamed')}"
                    )
        ...
        for i in range(0, len(assignments_data), batch_size):
            ...
            for assignment_config in batch:
                try:
                    ...
                    # Create assignment
                    created_assignment = self.client.create_assignment(
    ...
    def analyze_course_structure(
        ...
        try:
            ...
            performance_metrics = {
                ...
                "avg_module_items": (
                    sum(len(m.get("items", [])) for m in modules) / len(modules)
                    if modules
                    else 0
                ),
    ...
    def get_deployment_stats(self) -> Dict[str, Any]:
        ...
        partial = sum(
            1 for d in self.deployment_history if d.status == DeploymentStatus.PARTIAL
        )
    ...
    def _validate_course_data(
        self, course_data: Dict[str, Any], level: ValidationLevel
    ...
    def _validate_module_config(
        self, config: Dict[str, Any], level: ValidationLevel = ValidationLevel.STANDARD
    ) -> ValidationResult:
        """Validate module configuration."""
    ...
    def _setup_prerequisites(
        self, prerequisites: Dict[str, Any], course_id: Optional[str] = None
    ):
        ...
        for action in reversed(rollback_actions):  # Reverse order for proper cleanup
            try:
                if action["action"] == "delete_module":
                    endpoint = (
                        f"courses/{action['course_id']}/modules/{action['module_id']}"
                    )
                    ...
                    endpoint = (
                        f"courses/{action['course_id']}/pages/{action['page_id']}"
                    )
                    self.client.delete(endpoint)
    ...
    def _calculate_structure_score(
        self, modules: List[Dict], assignments: List[Dict]
    ) -> float:
        ...
        if assignments:
            ...
            assignment_types = set(
                a.get("submission_types", ["none"])[0] for a in assignments
            )
    ...
    def _calculate_engagement_score(
        ...
        interactive_assignments = [
            ...
            if any(
                t in a.get("submission_types", [])
                for t in ["online_quiz", "discussion_topic", "online_upload"]
            )
        ]
        ...
        has_discussions = any(
            "discussion" in a.get("submission_types", []) for a in assignments
        )
        has_projects = any(
            "upload" in a.get("submission_types", []) for a in assignments
        )
    ...
    def _generate_recommendations(
        ...
        if structure_score < 70:
            recommendations.append(
                "Consider reorganizing course modules for better logical flow"
            )
            if len(modules) > 20:
                recommendations.append(
                    "Too many modules may overwhelm students - consider consolidating"
                )
            elif len(modules) < 5:
                recommendations.append(
                    "Add more modules to break content into digestible chunks"
                )

        if accessibility_score < 80:
            recommendations.append(
                "Review content for accessibility compliance (alt text, headings, color contrast)"
            )
            recommendations.append("Consider adding captions to video content")

        if engagement_score < 60:
            recommendations.append(
                "Add more interactive elements (discussions, quizzes, projects)"
            )
            recommendations.append(
                "Consider implementing gamification elements (badges, progress tracking)"
            )
            recommendations.append(
                "Diversify assignment types to accommodate different learning styles"
            )
        ...
        if len(assignment_types) < 3:
            recommendations.append(
                "Add variety to assignment types to support different learning preferences"
            )

        if not any("discussion" in types for types in assignment_types):
            recommendations.append(
                "Consider adding discussion forums to encourage peer interaction"
            )
