"""
🎮 Test Script for Unified Gamification Engine
"""
...
from gamification_engine.pets.companion_system import (
    MathPetSystem,
    PetType,
    PetPersonality,
)
from gamification_engine.social.guild_system import GuildManager, Guild, StudyParty
from gamification_engine.social.mentorship_system import (
    PeerTeachingEngine,
    MentorshipChain,
)
...
from gamification_engine.world import WorldBuilder, StudyBase, ExplorationQuest
from gamification_engine.events import RealTimeEventEngine, LiveEvent, Challenge
...
class GamificationEngine:
    ...
    def __init__(self):
        ...
        self.rpg_system = PlayerProfileManager()
    ...
    def onboard_student(
        self, student_id: str, canvas_user_id: str = None, preferences: Dict = None
    ) -> bool:
        ...
        profile = StudentGameProfile(
            student_id=student_id, canvas_user_id=canvas_user_id
        )
        self.student_profiles[student_id] = profile

        # Initialize RPG system
        player_class = (
            preferences.get("player_class", MathematicalSpecialization.ENGINEER)
            if preferences
            else MathematicalSpecialization.ENGINEER
        )
        ...
        # Create study base
        base_name = (
            preferences.get("base_name", f"{student_id}'s Study Base")
            if preferences
            else f"{student_id}'s Study Base"
        )
    ...
    def _give_welcome_rewards(self, student_id: str):
        ...
        # Welcome badges
        profile.badges.extend(
            ["New Adventurer", "First Steps", "Mathematical Explorer"]
        )
    ...
    def award_xp(
        self, student_id: str, base_xp: int, source: str, context: Dict = None
    ) -> int:
        ...
        if profile.has_pet:
            ...
            if pet_info and pet_info.get("active_pet"):
        ...
        logger.info(
            f"Awarded {final_xp} XP to {student_id} from {source} (multiplier: {total_multiplier:.2f})"
        )
        return final_xp
    ...
    def complete_learning_activity(
        self, student_id: str, activity_type: str, performance_data: Dict
    ) -> Dict:
        ...
        if profile.rpg_active:
            ...
            if activity_type in ["matrix_operations", "determinant_calculation"]:
                self.rpg_system.award_skill_experience(
                    student_id, SkillCategory.MATRIX_MASTERY, skill_xp
                )
            elif activity_type in ["vector_operations", "dot_product"]:
                self.rpg_system.award_skill_experience(
                    student_id, SkillCategory.VECTOR_OPERATIONS, skill_xp
                )

        # Check for cross-system achievements
        achievements = self._check_cross_system_achievements(
            student_id, activity_type, performance_data
        )
    ...
    def _check_cross_system_achievements(
        ...
        if (
            profile.rpg_active
            and profile.has_pet
            and profile.guild_member
            and "Triple Threat" not in profile.achievements
        ):
        ...
        systems_active = sum(
            [
                ...
                profile.world_builder,
                profile.events_participant,
            ]
        )
    ...
    def get_comprehensive_dashboard(self, student_id: str) -> Dict:
        ...
        profile = self.student_profiles[student_id]
        dashboard = {"profile": asdict(profile), "systems": {}}
...
# Test the unified gamification engine
if __name__ == "__main__":
    ...
    preferences = {
        "player_class": MathematicalSpecialization.DATA_SCIENTIST,
        "wants_pet": True,
    ...
    for activity_type, performance in activities:
        result = engine.complete_learning_activity(
            "alice_complete", activity_type, performance
        )
    ...
    print(f"   World Builder: {dashboard['profile']['world_builder']}")
