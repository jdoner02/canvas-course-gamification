...
def test_economy_system():
    ...
    try:
        from gamification_engine.economy.currency_system import CurrencyManager, CurrencyType
...
def test_ai_content_engine():
    ...
    try:
        from gamification_engine.ai_content_engine.adaptive_learning import (
            AdaptiveLearningEngine, ConceptCategory, DifficultyLevel
        )
        ...
        engine.record_activity(
            "alice_123", "problem_solving", 
            ConceptCategory.VECTORS, DifficultyLevel.MEDIUM,
            True, 120.0
        )
...
def test_analytics_engine():
    ...
    try:
        from gamification_engine.analytics_engine.learning_analytics import (
            LearningAnalyticsEngine, AnalyticsMetric
        )
        ...
        analytics.record_analytics_event(
            "alice_123", "problem_completed",
            {"success": True, "concept": "vectors", "difficulty": "medium"}
        )
        
        analytics.record_analytics_event(
            "alice_123", "session_start",
...
def test_existing_systems():
    ...
    try:
        # Test core systems
        from gamification_engine.core.player_profile import PlayerProfile, MathematicalSpecialization, Skill, SkillCategory
        ...
        vector_skill = Skill("vector_ops", "Vector Operations", "Basic vector operations", SkillCategory.VECTOR_OPERATIONS)
        profile.skills["vector_ops"] = vector_skill
        
        # Add experience to the skill
        result = vector_skill.add_experience(100, MathematicalSpecialization.DATA_SCIENTIST)
        print(f"  ✅ RPG profile: Level {vector_skill.current_level}, XP {vector_skill.current_xp}, Gained {result['levels_gained']} levels")
        
        # Test pet system
        from gamification_engine.pets.companion_system import PetSpecies
        pet = MathematicalPet("pet_001", PetSpecies.VECTOR_SPRITE, "Vectra", "alice_123")
        pet.feed()
        print(f"  ✅ Pet system: {pet.name} happiness {pet.stats.happiness}")
        
        # Test guild system
        guild_manager = GuildManager()
        guild_id = guild_manager.create_guild("alice_123", "Linear Legends", "Masters of matrices")
...
def test_integration():
    ...
    try:
        ...
        from gamification_engine.ai_content_engine.adaptive_learning import AdaptiveLearningEngine, ConceptCategory, DifficultyLevel
        from gamification_engine.analytics_engine.learning_analytics import LearningAnalyticsEngine
        ...
        analytics.record_analytics_event(student_id, "session_start", {"session_type": "practice"})
