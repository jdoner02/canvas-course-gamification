...
class CanvasAPITester:
    ...
    def setup_session(self):
        ...
        if not token or token == "YOUR_CANVAS_API_TOKEN_HERE":
            ...
            logger.info(
                "Please set CANVAS_API_TOKEN environment variable or update config file"
            )
            sys.exit(1)
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✅ Connected successfully as: {user_info.get('name', 'Unknown')}"
            )
    ...
    def analyze_course(self, course_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for assignment in assignments:
                ...
                assignment_types[assignment_group]["assignments"].append(
                    assignment["name"]
                )
            ...
            for group_id, data in assignment_types.items():
                logger.info(
                    f"  Group {group_id}: {data['count']} assignments, {data['total_points']} points"
                )
    ...
    def update_student_xp(
        self, course_id: int, column_id: int, student_id: int, xp_value: int
    ) -> bool:
        ...
        try:
            ...
            data = {"column_data": {"content": str(xp_value)}}
...
def main():
    ...
    # Check for API token
    if (
        not os.getenv("CANVAS_API_TOKEN")
        and not Path("config/canvas_integration.yml").exists()
    ):
        print("⚠️  Canvas API token not found!")
        print("Please either:")
        print("1. Set CANVAS_API_TOKEN environment variable, or")
        print(
            "2. Copy config/canvas_integration_template.yml to config/canvas_integration.yml"
        )
