...
class WorkflowManager:
    ...
    def analyze_changes(self) -> Dict:
        ...
        for file in all_files:
            ...
            elif any(keyword in file.lower() for keyword in ['config', 'yml', 'yaml', 'json']):
                categories['config'].append(file)
            elif any(keyword in file.lower() for keyword in ['test', 'spec']):
                categories['test'].append(file)
            elif any(keyword in file.lower() for keyword in ['script', 'automation', 'cli']):
                categories['automation'].append(file)
            elif any(keyword in file.lower() for keyword in ['accessibility', 'wcag', 'udl']):
    ...
    def generate_commit_message(self, changes: Dict, issue_number: Optional[str] = None) -> str:
        ...
        # Determine scope
        scope = 'core'
        if any('canvas' in f.lower() for f in changes['staged'] + changes['unstaged'] + changes['untracked']):
            scope = 'canvas'
        elif any('gamification' in f.lower() for f in changes['staged'] + changes['unstaged'] + changes['untracked']):
            scope = 'gamification'
        elif any('validation' in f.lower() for f in changes['staged'] + changes['unstaged'] + changes['untracked']):
            scope = 'validation'
        elif any('cli' in f.lower() for f in changes['staged'] + changes['unstaged'] + changes['untracked']):
            scope = 'cli'
        ...
        if create_pr:
            ...
            try:
                result = subprocess.run([
                    'gh', 'pr', 'create',
                    '--title', f'Project Management and Bug Fixes - {datetime.now().strftime("%Y-%m-%d")}',
                    '--body', 'Automated PR for project management improvements and critical bug fixes'
    ...
    def display_status(self):
        ...
        status_table.add_row("Staged", str(len(changes['staged'])), 
                           ', '.join(changes['staged'][:3]) + ('...' if len(changes['staged']) > 3 else ''))
        status_table.add_row("Unstaged", str(len(changes['unstaged'])), 
                           ', '.join(changes['unstaged'][:3]) + ('...' if len(changes['unstaged']) > 3 else ''))
        status_table.add_row("Untracked", str(len(changes['untracked'])), 
                           ', '.join(changes['untracked'][:3]) + ('...' if len(changes['untracked']) > 3 else ''))
...
def main(status, commit, push, pr, issue, check):
    ...
    if commit:
        ...
        if not wf.run_quality_checks():
            console.print("❌ Quality checks failed. Fix issues before committing.", style="red")
            sys.exit(1)
