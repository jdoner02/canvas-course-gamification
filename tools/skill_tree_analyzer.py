...
class SkillTreeAnalyzer:
    ...
    def _parse_skills(self):
            ...
            if isinstance(group_data, dict):
                for key, value in group_data.items():
                    if key == "sub_skills" and isinstance(value, list):
                        ...
                        if "sub_skills" in value and isinstance(
                            value["sub_skills"], list
                        ):
                        ...
                        parse_skill_group(
                            value, f"{parent_path}.{key}" if parent_path else key
                        )
        ...
        if "applications_advanced" in self.config.get("math231_skill_tree", {}):
            parse_skill_group(
                self.config["math231_skill_tree"]["applications_advanced"]
            )
    ...
    def generate_dependency_graph(
        self, output_path: str = None, layout: str = "hierarchical"
    ):
        ...
        plt.title(
            "MATH 231 Linear Algebra Skill Tree Dependency Graph",
            fontsize=16,
            fontweight="bold",
            pad=20,
        )
        ...
        # Add legend
        legend_elements = [
            patches.Patch(facecolor=color, label=level.title())
...
"""
        ...
        prereq_counts = [
            (skill_id, len(self.skills[skill_id].get("prerequisites", [])))
            for skill_id in self.skills
        ]
        ...
        dependent_counts = [
            (skill_id, self.graph.out_degree(skill_id)) for skill_id in self.skills
        ]
...
def main():
    ...
    parser.add_argument(
        "--validate", action="store_true", help="Validate skill tree for issues"
    )
